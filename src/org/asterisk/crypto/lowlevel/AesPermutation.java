...
/**
 * implements the AES permutation. The exact implementation may change to
 * benefit performance reasons.
 * <p>
 * Current implementation is the standard one, using 5 1KB tables for each
 * direction
 *
 * @author Sayantan Chakraborty
 */
public class AesPermutation {
    ...
    /**
     * computes the shiftrows and subbytes transform on one column of the AES
     * matrix
     *
     * @param x one column of the aes matrix, represented as a big-endian
     *          32-bit integer
     *
     * @return
     */
    public static int shiftSub(int x) {
        return (Te4[(x >>> 16) & 0xff] & 0xff000000)
                | (Te4[(x >>> 8) & 0xff] & 0x00ff0000)
                | (Te4[(x) & 0xff] & 0x0000ff00)
                | (Te4[(x >>> 24) & 0xff] & 0x000000ff);
    }

    public static int invMixColumns(int x) {
        return Td0[0xff & Te4[(x >>> 24) & 0xff]]
                ^ Td1[0xff & Te4[(x >>> 16) & 0xff]]
                ^ Td2[0xff & Te4[(x >>> 8) & 0xff]]
                ^ Td3[0xff & Te4[(x) & 0xff]];
    }

    public static int mixColumns(int x) {
        return Te0[Td4[(x >>> 24) & 0xff] & 0xff]
                ^ Te1[Td4[(x >>> 16) & 0xff] & 0xff]
                ^ Te2[Td4[(x >>> 8) & 0xff] & 0xff]
                ^ Te3[Td4[(x) & 0xff] & 0xff];
    }

    public static void aesRound(int[] message, int mOffset, int[] dest, int dstOffset, int rk0, int rk1, int rk2, int rk3) {
        dest[dstOffset + 0] = Te0[(message[mOffset + 0] >>> 24) & 0xff]
                ^ Te1[(message[mOffset + 1] >>> 16) & 0xff]
                ^ Te2[(message[mOffset + 2] >>> 8) & 0xff]
                ^ Te3[(message[mOffset + 3]) & 0xff]
                ^ rk0;

        dest[dstOffset + 1] = Te0[(message[mOffset + 1] >>> 24) & 0xff]
                ^ Te1[(message[mOffset + 2] >>> 16) & 0xff]
                ^ Te2[(message[mOffset + 3] >>> 8) & 0xff]
                ^ Te3[(message[mOffset + 0]) & 0xff]
                ^ rk1;

        dest[dstOffset + 2] = Te0[(message[mOffset + 2] >>> 24) & 0xff]
                ^ Te1[(message[mOffset + 3] >>> 16) & 0xff]
                ^ Te2[(message[mOffset + 0] >>> 8) & 0xff]
                ^ Te3[(message[mOffset + 1]) & 0xff]
                ^ rk2;

        dest[dstOffset + 3] = Te0[(message[mOffset + 3] >>> 24) & 0xff]
                ^ Te1[(message[mOffset + 0] >>> 16) & 0xff]
                ^ Te2[(message[mOffset + 1] >>> 8) & 0xff]
                ^ Te3[(message[mOffset + 2]) & 0xff]
                ^ rk3;
    }

    /**
     * computes one round of the aes round function(not the last one)
     * <p>
     * the parameters are each the aes matrix, represented as an array of
     * columns, each represented as a big-endian 32-bit integer
     * <p>
     * the other overloads of this method concerns with destructuring the
     * message and round key array for better use in some cases, like pre-xoring
     * the state with a fixed word and many more(we found it to
     * be very helpful)
     * <p>
     * each round takes around 37 cycles to compute
     * <p>
     * The destructured variants also have another use. They facilitate reusing
     * the same state array. In general, if you destructure a specific input,
     * it can freely overlap with the destination. Otherwise, message words
     * cannot overlap at all with the destination, and round key words can be at
     * a position greater than or equal to the destination (using pointer
     * comparison)
     *
     * @param message
     * @param mOffset
     ...
     * @param kOffset
     */
    public static void aesRound(int[] message, int mOffset, int[] dest, int dstOffset, int[] rk, int kOffset) {
        dest[dstOffset + 0] = Te0[(message[mOffset + 0] >>> 24) & 0xff]
                ^ Te1[(message[mOffset + 1] >>> 16) & 0xff]
                ^ Te2[(message[mOffset + 2] >>> 8) & 0xff]
                ^ Te3[(message[mOffset + 3]) & 0xff]
    ...
    public static void aesRound(int message0, int message1, int message2, int message3, int[] dest, int dstOffset, int[] rk, int kOffset) {
        dest[dstOffset + 0] = Te0[(message0 >>> 24) & 0xff]
                ^ Te1[(message1 >>> 16) & 0xff]
                ^ Te2[(message2 >>> 8) & 0xff]
                ^ Te3[(message3) & 0xff]
                ^ rk[kOffset + 0];

        dest[dstOffset + 1] = Te0[(message1 >>> 24) & 0xff]
                ^ Te1[(message2 >>> 16) & 0xff]
                ^ Te2[(message3 >>> 8) & 0xff]
                ^ Te3[(message0) & 0xff]
                ^ rk[kOffset + 1];

        dest[dstOffset + 2] = Te0[(message2 >>> 24) & 0xff]
                ^ Te1[(message3 >>> 16) & 0xff]
                ^ Te2[(message0 >>> 8) & 0xff]
                ^ Te3[(message1) & 0xff]
                ^ rk[kOffset + 2];

        dest[dstOffset + 3] = Te0[(message3 >>> 24) & 0xff]
                ^ Te1[(message0 >>> 16) & 0xff]
                ^ Te2[(message1 >>> 8) & 0xff]
                ^ Te3[(message2) & 0xff]
                ^ rk[kOffset + 3];
    }

    public static void aesRound(int message0, int message1, int message2, int message3, int[] dest, int dstOffset, int rk0, int rk1, int rk2, int rk3) {
        dest[dstOffset + 0] = Te0[(message0 >>> 24) & 0xff]
                ^ Te1[(message1 >>> 16) & 0xff]
                ^ Te2[(message2 >>> 8) & 0xff]
                ^ Te3[(message3) & 0xff]
    ...
    public static void aesRoundLast(int[] message, int mOffset, int[] dest, int dstOffset, int[] rk, int kOffset) {
        dest[dstOffset + 0] = (Te4[(message[mOffset + 0] >>> 24) & 0xff] & 0xff000000)
                ^ (Te4[(message[mOffset + 1] >>> 16) & 0xff] & 0x00ff0000)
                ^ (Te4[(message[mOffset + 2] >>> 8) & 0xff] & 0x0000ff00)
                ^ (Te4[(message[mOffset + 3]) & 0xff] & 0x000000ff)
                ^ rk[kOffset + 0];

        dest[dstOffset + 1] = (Te4[(message[mOffset + 1] >>> 24) & 0xff] & 0xff000000)
                ^ (Te4[(message[mOffset + 2] >>> 16) & 0xff] & 0x00ff0000)
                ^ (Te4[(message[mOffset + 3] >>> 8) & 0xff] & 0x0000ff00)
                ^ (Te4[(message[mOffset + 0]) & 0xff] & 0x000000ff)
                ^ rk[kOffset + 1];

        dest[dstOffset + 2] = (Te4[(message[mOffset + 2] >>> 24) & 0xff] & 0xff000000)
                ^ (Te4[(message[mOffset + 3] >>> 16) & 0xff] & 0x00ff0000)
                ^ (Te4[(message[mOffset + 0] >>> 8) & 0xff] & 0x0000ff00)
                ^ (Te4[(message[mOffset + 1]) & 0xff] & 0x000000ff)
                ^ rk[kOffset + 2];

        dest[dstOffset + 3] = (Te4[(message[mOffset + 3] >>> 24) & 0xff] & 0xff000000)
                ^ (Te4[(message[mOffset + 0] >>> 16) & 0xff] & 0x00ff0000)
                ^ (Te4[(message[mOffset + 1] >>> 8) & 0xff] & 0x0000ff00)
                ^ (Te4[(message[mOffset + 2]) & 0xff] & 0x000000ff)
                ^ rk[kOffset + 3];

    }

    public static void invAesRound(int[] message, int mOffset, int[] dest, int dstOffset, int drk0, int drk1, int drk2, int drk3) {
        dest[dstOffset + 0] = Td0[(message[mOffset + 0] >>> 24) & 0xff]
                ^ Td1[(message[mOffset + 3] >>> 16) & 0xff]
                ^ Td2[(message[mOffset + 2] >>> 8) & 0xff]
                ^ Td3[(message[mOffset + 1]) & 0xff]
                ^ drk0;

        dest[dstOffset + 1] = Td0[(message[mOffset + 1] >>> 24) & 0xff]
                ^ Td1[(message[mOffset + 0] >>> 16) & 0xff]
                ^ Td2[(message[mOffset + 3] >>> 8) & 0xff]
                ^ Td3[(message[mOffset + 2]) & 0xff]
                ^ drk1;

        dest[dstOffset + 2] = Td0[(message[mOffset + 2] >>> 24) & 0xff]
                ^ Td1[(message[mOffset + 1] >>> 16) & 0xff]
                ^ Td2[(message[mOffset + 0] >>> 8) & 0xff]
                ^ Td3[(message[mOffset + 3]) & 0xff]
                ^ drk2;

        dest[dstOffset + 3] = Td0[(message[mOffset + 3] >>> 24) & 0xff]
                ^ Td1[(message[mOffset + 2] >>> 16) & 0xff]
                ^ Td2[(message[mOffset + 1] >>> 8) & 0xff]
                ^ Td3[(message[mOffset + 0]) & 0xff]
                ^ drk3;
    }

    public static void invAesRound(int[] message, int mOffset, int[] dest, int dstOffset, int[] drk, int kOffset) {
        dest[dstOffset + 0] = Td0[(message[mOffset + 0] >>> 24) & 0xff]
                ^ Td1[(message[mOffset + 3] >>> 16) & 0xff]
                ^ Td2[(message[mOffset + 2] >>> 8) & 0xff]
                ^ Td3[(message[mOffset + 1]) & 0xff]
    ...
    public static void invAesRound(int message0, int message1, int message2, int message3, int[] dest, int dstOffset, int[] drk, int kOffset) {
        dest[dstOffset + 0] = Td0[(message0 >>> 24) & 0xff]
                ^ Td1[(message3 >>> 16) & 0xff]
                ^ Td2[(message2 >>> 8) & 0xff]
                ^ Td3[(message1) & 0xff]
                ^ drk[kOffset + 0];

        dest[dstOffset + 1] = Td0[(message1 >>> 24) & 0xff]
                ^ Td1[(message0 >>> 16) & 0xff]
                ^ Td2[(message3 >>> 8) & 0xff]
                ^ Td3[(message2) & 0xff]
                ^ drk[kOffset + 1];

        dest[dstOffset + 2] = Td0[(message2 >>> 24) & 0xff]
                ^ Td1[(message1 >>> 16) & 0xff]
                ^ Td2[(message0 >>> 8) & 0xff]
                ^ Td3[(message3) & 0xff]
                ^ drk[kOffset + 2];

        dest[dstOffset + 3] = Td0[(message3 >>> 24) & 0xff]
                ^ Td1[(message2 >>> 16) & 0xff]
                ^ Td2[(message1 >>> 8) & 0xff]
                ^ Td3[(message0) & 0xff]
                ^ drk[kOffset + 3];
    }

    public static void invAesRound(int message0, int message1, int message2, int message3, int[] dest, int dstOffset, int drk0, int drk1, int drk2, int drk3) {
        dest[dstOffset + 0] = Td0[(message0 >>> 24) & 0xff]
                ^ Td1[(message3 >>> 16) & 0xff]
                ^ Td2[(message2 >>> 8) & 0xff]
                ^ Td3[(message1) & 0xff]
    ...
    public static void invAesRoundLast(int[] message, int mOffset, int[] dest, int dstOffset, int[] drk, int kOffset) {
        dest[dstOffset + 0] = (Td4[(message[mOffset + 0] >>> 24) & 0xff] & 0xff000000)
                ^ (Td4[(message[mOffset + 3] >>> 16) & 0xff] & 0x00ff0000)
                ^ (Td4[(message[mOffset + 2] >>> 8) & 0xff] & 0x0000ff00)
                ^ (Td4[(message[mOffset + 1]) & 0xff] & 0x000000ff)
                ^ drk[kOffset + 0];

        dest[dstOffset + 1] = (Td4[(message[mOffset + 1] >>> 24) & 0xff] & 0xff000000)
                ^ (Td4[(message[mOffset + 0] >>> 16) & 0xff] & 0x00ff0000)
                ^ (Td4[(message[mOffset + 3] >>> 8) & 0xff] & 0x0000ff00)
                ^ (Td4[(message[mOffset + 2]) & 0xff] & 0x000000ff)
                ^ drk[kOffset + 1];

        dest[dstOffset + 2] = (Td4[(message[mOffset + 2] >>> 24) & 0xff] & 0xff000000)
                ^ (Td4[(message[mOffset + 1] >>> 16) & 0xff] & 0x00ff0000)
                ^ (Td4[(message[mOffset + 0] >>> 8) & 0xff] & 0x0000ff00)
                ^ (Td4[(message[mOffset + 3]) & 0xff] & 0x000000ff)
                ^ drk[kOffset + 2];

        dest[dstOffset + 3] = (Td4[(message[mOffset + 3] >>> 24) & 0xff] & 0xff000000)
                ^ (Td4[(message[mOffset + 2] >>> 16) & 0xff] & 0x00ff0000)
                ^ (Td4[(message[mOffset + 1] >>> 8) & 0xff] & 0x0000ff00)
                ^ (Td4[(message[mOffset + 0]) & 0xff] & 0x000000ff)
                ^ drk[kOffset + 3];
    }

    private AesPermutation() {
    }
